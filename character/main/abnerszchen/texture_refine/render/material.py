...
def load_mtl(fn, clear_ks=True):
    ...
    # Parse materials
    materials = []
    skip_keys = ['map_d', 'map_ke'] # for readyplayme
    for line in lines:
        split_line = re.split(' +|\t+|\n+', line.strip())
        ...
        if 'newmtl' in prefix:
            ...
            if prefix in skip_keys:
                continue
            if 'bsdf' in prefix or 'map_kd' in prefix or 'map_ks' in prefix or 'bump' in prefix or 'map_bump' in prefix:
                if 'map_bump' in prefix:    # for readyplayme
                    images = [v for v in data if os.path.splitext(v)[-1] in ['.jpg', '.png']]
                    material['bump'] = images[0]
                elif "map_refl" in prefix:    # for daz
                    pass
                else:
                    if len(data) == 1:
                        material[prefix] = data[0]  # map_kd image texture
                    else:
                        material[prefix] = " ".join(data)  # hard code for daz, TODO
            else:
                material[prefix] = torch.tensor(tuple(float(d) for d in data), dtype=torch.float32, device='cuda')  # kd number texture
