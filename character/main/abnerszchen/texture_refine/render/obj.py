...
def load_obj(filename, clear_ks=True, mtl_override=None, skip_mtl=False):
...
def write_obj(folder, mesh, save_material=True):
    ...
    with open(obj_file, "w") as f:
        ...
        for i in range(len(t_pos_idx)):
            f.write("f ")
            # fix for pytorch3d when no v_nrm
            if v_nrm is None:
                for j in range(3):
...
def new_texed_mesh(in_mesh, new_tex_np):
    new_mesh = in_mesh.clone()
    new_mesh.material = in_mesh.material
    new_mesh.material['kd'] = texture.Texture2D(new_tex_np)
    return new_mesh    
