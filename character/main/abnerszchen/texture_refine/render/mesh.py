...
def load_mesh(filename, mtl_override=None, skip_mtl=False):
    name, ext = os.path.splitext(filename)
    if ext == ".obj":
        return obj.load_obj(filename, clear_ks=True, mtl_override=mtl_override, skip_mtl=skip_mtl)
    assert False, "Invalid mesh file extension"


def auto_center(mesh, scale_factor=1.0):
    # torch.min(mesh.v_pos, dim=0).values, torch.max(mesh.v_pos, dim=0).values
    verts = mesh.v_pos
    ...
    v_pos = verts - center
    mesh.v_pos = v_pos * (scale_factor / float(scale))
    return mesh
