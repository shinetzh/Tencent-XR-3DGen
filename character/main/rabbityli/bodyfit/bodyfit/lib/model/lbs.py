...
from .index import smplx_vert_segmentation, neck_head_border_verts
...
def lbs(betas, pose, v_template, shapedirs, posedirs, J_regressor, parents,
        lbs_weights, pose2rot=True, head_scale=None, body_scale=None, dtype=torch.float32):
    ...
    #add head scale contribution
    scale_offset = torch.zeros_like( shape_offsets )
    neck_center = v_shaped[:, neck_head_border_verts]
    neck_center = neck_center.mean(dim=1, keepdim=True)
    if head_scale:
        head_vid = []
        for key in ["head", "rightEye",  "leftEye", "eyeballs"]:
            head_vid = head_vid + smplx_vert_segmentation[ key ]
        head_verts = v_shaped[ :, head_vid ]
        head_verts = head_verts - neck_center
        scaled_verts = head_scale * head_verts
        head_scale_offset = scaled_verts - head_verts
        # v_shaped[ :,head_vid ] = v_shaped[ :, head_vid ] + head_scale_offset
        scale_offset [:,head_vid] = head_scale_offset
    if body_scale:
        body_vid = []
        for key in smplx_vert_segmentation.keys():
            if key not in ["head", "rightEye",  "leftEye", "eyeballs"]:
                body_vid = body_vid + smplx_vert_segmentation[ key ]
        body_verts = v_shaped[:, body_vid]
        body_verts = body_verts - neck_center
        scaled_verts = body_scale * body_verts
        body_scale_offset = scaled_verts - body_verts
        # v_shaped[:, body_vid] = v_shaped[:, body_vid] + body_scale_offset
        scale_offset [:,body_vid] = body_scale_offset
    v_shaped = v_shaped + scale_offset
    ...
    if pose2rot:
        ...
        # print("test", pose_feature.device, posedirs.device)
    ...
    return verts, J_transformed, A, T, shape_offsets, scale_offset, pose_offsets
