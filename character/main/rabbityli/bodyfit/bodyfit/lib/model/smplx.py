...
class SMPLX(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.cfg = config
        # with open(config.smplx_model_path, 'rb') as f:
        #     ss = pickle.load(f)
        ...
        self.register_buffer('transl', nn.Parameter(torch.zeros([1, 3], dtype=self.dtype), requires_grad=False))
    ...
    def forward(self, shape_params=None, expression_params=None,
                    ...
                    offset=None, transl=None, return_T = False, head_scale=None, body_scale=None,):
        """
            Args:
        ...
        # smplx
        vertices, joints, A, T, shape_offsets, scale_offset, pose_offsets = lbs(shape_components, full_pose, template_vertices,
                          self.shapedirs, self.posedirs,
                          self.J_regressor, self.parents,
                          self.lbs_weights, dtype=self.dtype,
                          pose2rot = False, head_scale=head_scale, body_scale=body_scale)
        ...
        if transl is not None:
            ...
            joints = joints + self.transl.unsqueeze(dim=1)
            vertices = vertices + self.transl.unsqueeze(dim=1)
            if return_T:
                A[..., :3, 3] += self.transl.unsqueeze(dim=1)
                T[..., :3, 3] += self.transl.unsqueeze(dim=1)

        if return_T:
            return vertices, landmarks, joints, A, T, shape_offsets, scale_offset, pose_offsets
        else:
    ...
    def set_canonical_space(self, pose):
        ...
        canonical_verts, _, _, A, T, shape_offsets, scale_offset, pose_offsets = self.forward(full_pose=pose, return_T=True)
        verts = canonical_verts.squeeze()
        faces = self.faces_tensor.squeeze()
        # record transform for later use
        canonical_transform = T
        canonical_offsets = shape_offsets + scale_offset + pose_offsets
    ...
    def backward_skinning(self, full_pose, shape_params,body_pose=None, global_pose=None, exp=None, offset=None, transl=None):
        ...
        posed_verts, _, _, joints_transform, curr_vertices_transform, shape_offsets, scale_offset, pose_offsets = \
                    self.forward(full_pose=full_pose, 
                                ...
                                transl=transl,
                                return_T=True)
        curr_offsets = shape_offsets + scale_offset+ pose_offsets
    ...
    @torch.cuda.amp.autocast(enabled=False)
    def forward_skinning(self, full_pose=None, shape_params=None, body_pose = None,  global_pose = None, exp=None, offset=None, transl=None, head_scale=None, body_scale=None,):
        ...
        full_offsets = shape_offsets + scale_offset + pose_offsets


        #compute full vertices transform from template -->  canonical --> posed
        Rot = curr_vertices_transform [..., :3, :3]
        trn = curr_vertices_transform [..., :3, 3:]
        trn_full = trn + Rot @ full_offsets[..., None]
        vertices_transform = torch.cat( [Rot, trn_full], dim=-1)
        vertices_transform = torch.cat( [vertices_transform, curr_vertices_transform [:,:, 3:] ], dim=-2)
