import os, sys, copy, json, torch, trimesh, pathlib, argparse

codedir = os.path.dirname(os.path.abspath(__file__))
sys.path.append(os.path.dirname(codedir))
...
base_body_map = load_json(os.path.join(codedir, "base_body_map.json"))
base_body_map_source = load_json(os.path.join(codedir, "base_body_map_source.json"))
indexs = str(Path(os.path.join(codedir)).parent / "bodyfit" / "smplx_vert_segmentation.json")
...
def main():
    ...
    key_smpl_map = load_json(os.path.join(codedir, "20240711_key_smpl_map.json"))
    ...
    # print(base_body_map)
    target_dir = base_body_map [body_info[0]][body_info[1]]["path"].replace("/aigc_cfs/rabbityli", "/aigc_cfs_gdp/Asset")
    ...
    for idx, part in enumerate(part_info):
        ...
        if body_key[1] in ["daz", "vroid"]:
            ...
            nake_dir = base_body_map_source[body_key[0]][body_key[1]]["path"].replace("/aigc_cfs/rabbityli", "/aigc_cfs_gdp/Asset")
        ...
        if not os.path.exists(part):
            ...
            for key_type in total_data["data"]:
                for key in total_data["data"][key_type]:
                    if key == asset_key:
                        part = total_data["data"][key_type][key]["Obj_Mesh"]
        print("part", part)
    ...
    with open(os.path.join(dump_root, "wrap_cloth.txt"), "w") as f:
        f.write(f"test\n")
