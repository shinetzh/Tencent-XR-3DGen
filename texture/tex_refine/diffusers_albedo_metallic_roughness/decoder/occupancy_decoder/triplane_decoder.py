...
from decoder.occupancy_decoder.decoder_net import SdfDecoderYh
...
class OccupancyModel(nn.Module):
    def __init__(self, configs_json, type="occupancy"):
        super(OccupancyModel, self).__init__()

        with open(configs_json, encoding='utf-8') as f:
            self.dataset_dict = edict(json.load(f))

        configs = edict(self.dataset_dict["config"]["Config"])

        d_in = configs.channels
        d_out = configs.part_num
        d_hidden = configs.width
        ...
        ckpt_path = self.dataset_dict["config"]["MLP"]
        self.load_state_dict(torch.load(ckpt_path, map_location='cuda'))
        print(f"decoder loaded ckpt from {ckpt_path}")
        self.net.eval()
        print(self.net)
    ...
    def forward_with_plane_features(self, plane_features, xyz):
        ...
        '''
        point_features = self.get_points_plane_features(plane_features, xyz) # point_features: B, N, D
        pred_sdf = self.net(point_features)
        return pred_sdf # [B, num_points] 
