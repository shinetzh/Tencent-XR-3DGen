...
class DatasetModTransfer_metallic(Dataset):
    def __init__(self,
        configs,
        data_type = "train",
        bg_color="white",
    ...
    def __getitem__(self, index):
        ...
        for condi in cond_idx_list_use:
            ...
            normal_cond_path = os.path.join(image_dir, "metallic/color", f"cam-{str(image_cond_sub_idx).zfill(4)}.png")

            image = Image.open(image_cond_path)
            image = to_rgb_image(image, 255)
            image = self.train_transforms(image)

            normal = Image.open(normal_cond_path)
            normal = to_rgb_image(normal, 255)
            normal = self.train_transforms(normal)

            image_list.append(image[None])
            normal_list.append(normal[None])

        images = torch.cat(image_list)
        normals = torch.cat(normal_list)
...
if __name__ == "__main__":
    ...
    configs = {
                ...
                "data_config":{
                    "dataset_name" : "DatasetModTransfer_metallic",
                    ...
                    "image_size": 512,
                    "group_idx_list": [0, 1],
                    "cond_idx_list": [0,1,2,3],
                    "images_num_per_group": 24,
                    "load_from_cache_last": False
                }
            }

    train_dataset = DatasetModTransfer_metallic(configs, data_type="train", load_from_cache_last=False)
    ...
    for data in train_dataloader:
        ...
        normals = make_grid(normals[0], nrow=2, padding=0) * 0.5 + 0.5
        save_image(normals, "normals.jpg")
        breakpoint()
