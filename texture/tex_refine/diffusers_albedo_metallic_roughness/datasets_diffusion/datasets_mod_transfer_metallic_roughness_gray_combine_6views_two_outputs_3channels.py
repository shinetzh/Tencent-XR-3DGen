...
class DatasetModTransfer_metallic_roughness_two_outputs_3channels(Dataset):
    def __init__(self,
        configs,
        data_type = "train",
        bg_color="white",
    ...
    def read_data(self, data_dict):
        ...
        for objname, objdict in tqdm(data_dict.items()):
            ...
            for groupi in range(self.group_idx_range):
                ...
                all_data_list.append([image_dir, metallic_roughness_dir, groupi])
    ...
    def __getitem__(self, index):
        ...
        # breakpoint()
        image_dir, metallic_roughness_dir, groupi = self.all_objects[index]
        ...
        for condi in cond_idx_list_use:
            ...
            metallic = Image.open(metallic_cond_path)
            metallic = metallic.resize((self.image_size, self.image_size))
            metallic, _ = to_rgb_image(metallic, 127)
            # metallic = to_material_image_mask(metallic_cond_path, mask, resolution=self.image_size )
            metallic = self.train_transforms(metallic)

            roughness = Image.open(roughness_cond_path)
            roughness = roughness.resize((self.image_size, self.image_size))
            roughness, _ = to_rgb_image(roughness, 127)
...
if __name__ == "__main__":
    ...
    train_dataset = DatasetModTransfer_metallic_roughness_two_outputs_3channels(configs, data_type="train", load_from_cache_last=False)
    ...
    for data in train_dataloader:
        ...
        save_image(metallics, "metallics_" + str(i) + ".jpg")
