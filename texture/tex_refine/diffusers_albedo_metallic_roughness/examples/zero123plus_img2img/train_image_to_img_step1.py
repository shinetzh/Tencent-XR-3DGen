...
from src.diffusers.pipelines.zero123_diffusion.pipeline_zero123plus_img2img import Zero123PlusImg2ImgPipeline, RefOnlyNoisedUNet, scale_image, scale_latents, unscale_image, unscale_latents
from src.diffusers.pipelines.zero123_diffusion.pipeline_zero123plus import Zero123PlusPipeline
...
def log_validation(vae, text_encoder, vision_encoder, tokenizer, unet, unet_zero123plus, args, accelerator, weight_dtype, epoch, max_num=4):
    ...
    pipeline = Zero123PlusPipeline.from_pretrained(args.img2mv_pretrain_path, 
                                                   unet=unet_zero123plus,
    ...
    generator = torch.Generator(device=accelerator.device).manual_seed(99)
    ...
    for i in range(len(validation_image_path_list)):
        ...
        try:
            cond = Image.open(validation_image_path_list[i])
        except:
            continue
        image1 =  pipeline(cond, num_inference_steps=75, guidance_scale=3.0, generator=generator, output_type="pt")
        if image1 is None:
            continue
        result = pipeline_img2img(cond, image1.images[0], num_inference_steps=75, guidance_scale=3.0, output_type="pt", generator=generator)
...
def parse_args():
    ...
    parser.add_argument(
        "--img2mv_pretrain_path",
        type=str,
        default=None,
        required=True,
        help="Path to pretrained model or model of img2mv with zero123++.",
    )
...
def main():
    ...
    #     args.pretrained_model_name_or_path, subfolder="feature_extractor_clip", revision=args.revision
    # )
    # feature_extractor_vae = CLIPImageProcessor.from_pretrained(
    #     args.pretrained_model_name_or_path, subfolder="feature_extractor_clip", revision=args.revision
    # )
    ...
    _unet_ = UNet2DConditionModel.from_pretrained(args.pretrained_model_name_or_path, subfolder="unet", revision=args.non_ema_revision)
    unet = RefOnlyNoisedUNet(unet=_unet_, train_sched=noise_scheduler, val_sched=val_noise_scheduler)


    unet_zero123plus = UNet2DConditionModel.from_pretrained(args.img2mv_pretrain_path, subfolder="unet", revision=args.non_ema_revision)
    # unet_zero123plus = RefOnlyNoisedUNet(unet=_unet_zero123plus, train_sched=noise_scheduler, val_sched=val_noise_scheduler)
    unet_zero123plus.requires_grad_(False)
    ...
    # train_param_dict = {}
    # check_param_name = None
    # for name, param in filter(lambda p: p[1].requires_grad, unet.named_parameters()):
    #     train_param_dict[name] = param
    #     if check_param_name is None:
    #         check_param_name = name
    #         print(f"unt: {id(param)}")

    # register_params_for_train(train_module, train_param_dict)
    # for name, param in train_module.named_parameters():
    #     if name == check_param_name.replace(".", "-"):
    #         print(f"train_module: {id(param)}")
    #         break
    ...
    # dataset_validation = get_dataset(configs, data_type="test", resample=False)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                cond_image_clip = batch["cond_image_clip"].to(weight_dtype)
                cond_image_vae = batch["cond_image_vae"].to(weight_dtype)
                images_in = batch["images_in"].to(weight_dtype)
                images_out = batch["images_out"].to(weight_dtype)
                task_embeddings = batch["task_embeddings"].to(weight_dtype)

                images_out = scale_image(images_out)
                b, v, c, h, w = images_out.shape

                task_embeddings_pe = torch.cat([
                    torch.sin(task_embeddings),
                    torch.cos(task_embeddings)
                ], dim=-1)

                ### get latents out
                latents_out = vae.encode(images_out.view(b*v, c, h, w).to(weight_dtype)).latent_dist.sample()
                _, cl, hl, wl = latents_out.shape
                latents_out = latents_out.view(b, v, cl, hl, wl)
                ncol = v // 2
                nrow = v // ncol
                ...
                if args.do_classifier_free_guidance:
                    drop_idx = np.where(np.random.rand(cond_image_vae.shape[0])<0.15)
                    cond_image_vae[drop_idx] = 0.0
                    images_in[drop_idx] = 0.0

                ### get latents in
                latents_in = vae.encode(images_in.view(b*v, c, h, w).to(weight_dtype)).latent_dist.sample()
                _, cl, hl, wl = latents_in.shape
                latents_in = latents_in.view(b, v, cl, hl, wl)
                ncol = v // 2
                nrow = v // ncol
                latents_in_grid = torch.zeros((b, cl, hl*ncol, wl*nrow)).to(latents_in.device, dtype=weight_dtype)
                for i in range(b):
                    latents_in_grid[i] = make_grid(latents_in[i], nrow=nrow, padding=0)
                # assert torch.sum(latents_in_grid[2, :, 40:, :40] - latents_in[2, 2, ...]) == 0
                latents_in_grid = latents_in_grid * vae.config.scaling_factor
                ...
                latents_cond_vae = torch.cat([latents_cond_vae]*2, dim=1)
                ...
                noisy_latents = torch.cat([noisy_latents, latents_in_grid], dim=1)
                ...
                if noise_scheduler.config.prediction_type == "epsilon":
                    ...
                    target = noise_scheduler.get_velocity(latents_out_grid, noise, timesteps)
                else:
                ...
                model_pred = unet(
                    ...
                    class_labels=task_embeddings_pe,
                ).sample
            ...
            if global_step >= args.max_train_steps:
                break
        # if accelerator.is_main_process:
        #     if args.validation_images_dir is not None and epoch % args.validation_epochs == 0:
        #         if args.use_ema:
        #             # Store the UNet parameters temporarily and load the EMA parameters to perform inference.
        #             ema_unet.store(unet.parameters())
        #             ema_unet.copy_to(unet.parameters())
    ...
    if accelerator.is_main_process:
        ...
        pipeline = Zero123PlusImg2ImgPipeline.from_pretrained(
            args.pretrained_model_name_or_path,
