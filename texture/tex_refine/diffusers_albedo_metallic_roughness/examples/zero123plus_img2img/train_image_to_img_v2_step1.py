...
from src.diffusers.pipelines.zero123_diffusion.pipeline_zero123plus_img2img_v2 import Zero123PlusImg2ImgPipeline, ImageConditionUNet, scale_image, scale_latents, unscale_image, unscale_latents
...
def log_validation(vae, text_encoder, vision_encoder, tokenizer, unet, args, accelerator, weight_dtype, epoch, max_num=8):
    ...
    for filename in os.listdir(args.validation_images_dir):
        ...
        shading_image_cat_path = os.path.join(args.validation_images_mv_shading_dir, filename)
        if os.path.exists(shading_image_cat_path):
            validation_image_path_list.append([filepath, shading_image_cat_path])

    for i in range(len(validation_image_path_list)):
        if len(images) >= max_num:
            break
        print(f"\rvalidate: {i}/{max_num}", end="", flush=True)
        cond_path = validation_image_path_list[i][0]
        mv_path = validation_image_path_list[i][1]
        cond = process_image_path(cond_path, bg_color=255, wh_ratio=0.8)
        shading_image_cat = np.array(Image.open(mv_path))
        ...
        result = pipeline(cond, images_in_list, num_inference_steps=75, guidance_scale=3.0,  output_type="pt", generator=generator)
...
def parse_args():
    ...
    parser.add_argument(
        "--validation_images_mv_shading_dir",
        type=str,
        default=None,
        help=(
            "validation images mv concat images direction"),
    )
...
def main():
    ...
    unet = ImageConditionUNet(unet=_unet_, train_sched=noise_scheduler, val_sched=val_noise_scheduler)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                images_in = batch["images_in"]
                images_out = batch["images_out"]
                images_in = batch["images_in"]
                class_embeddings = batch["task_embeddings"]
                images_in = scale_image(images_in)
                ...
                prompt_embedding = tokenize_captions([""]).to(cond_image_clip.device)
                ...
                # print(batch["cond_image_path"])
                # torch.save(latents_in_grid, "latents_in_grid.pt")
                # torch.save(class_embeddings, "class_embeddings.pt")
                # torch.save(cond_encoded_clip, "cond_encoded_clip.pt")
                # torch.save(encoder_hidden_states_prompt, "encoder_hidden_states_prompt.pt")
                
                # breakpoint()


                if args.do_classifier_free_guidance:
                    # negative_cond_image_vae = torch.zeros_like(cond_image_vae, dtype=weight_dtype)
                    drop_idx = np.where(np.random.rand(cond_image_clip.shape[0])<0.2)
                    latents_in_grid[drop_idx] = 0
                ...
                concatenated_noisy_latents = torch.cat([noisy_latents, latents_in_grid], dim=1)
                
                model_pred = unet(
                    concatenated_noisy_latents,
                    timesteps,
                    class_labels=class_embeddings,
