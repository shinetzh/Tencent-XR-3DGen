...
def log_validation(vae, text_encoder, vision_encoder, tokenizer, unet, args, accelerator, weight_dtype, epoch, max_num=8):
    ...
    zero123plus_pretrained_dir = "/aigc_cfs_2/neoshang/code/diffusers_triplane/release/zero23plus_v4.4"
    pipeline = Zero123PlusPipeline.from_pretrained(zero123plus_pretrained_dir, 
    ...
    for i in range(len(validation_image_path_list)):
        ...
        image1 =  pipeline(cond, num_inference_steps=75, guidance_scale=3.0, generator=generator).images[0]
        print("zero123++ image to image running...")
        result = pipeline_img2img(cond, image1, num_inference_steps=75, guidance_scale=3.0, output_type="pt", generator=generator).images[0]
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if args.do_classifier_free_guidance:
                    ...
                    negative_images_in_vae = torch.zeros_like(images_in, dtype=weight_dtype)
                    drop_idx = np.where(np.random.rand(cond_image_vae.shape[0])<0.15)
                    cond_image_vae[drop_idx] = negative_cond_image_vae[drop_idx]
                    images_in[drop_idx] = negative_images_in_vae[drop_idx]

                ### get latents in
                latents_in_grid = vae.encode(images_in.to(weight_dtype)).latent_dist.sample()
                ...
                noisy_latents = torch.cat([latents_in_grid, noisy_latents], dim=1)
