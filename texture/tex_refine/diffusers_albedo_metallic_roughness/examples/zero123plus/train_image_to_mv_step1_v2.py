...
def main():
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if args.snr_gamma is None:
                    ...
                    if loss >= 0.02:
                        loss_weight = loss.detach()
                        loss = math.pow(10000, -loss_weight) * loss
        ...
        if accelerator.is_main_process:
            ...
            loss_min = loss_list_new[0]
            loss_max = loss_list_new[len(loss_list_new)-1]
            loss_medium = loss_list_new[int(len(loss_list_new)//2)]
            logger.info(f"\n\tloss min: {loss_min}\n\tloss max: {loss_max}\n" \
                        f"\tloss mean: {loss_mean}\n\tloss_midium: {loss_medium}\n\tloss var: {loss_var}")
