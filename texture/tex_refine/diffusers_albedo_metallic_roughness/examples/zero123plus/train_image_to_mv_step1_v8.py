...
def log_validation(vae, text_encoder, vision_encoder, tokenizer, unet, args, accelerator, weight_dtype, epoch, max_num=8):
    ...
    for i in range(len(validation_image_path_list)):
        ...
        result = pipeline(cond, num_inference_steps=75, guidance_scale=3.0, output_type="pt", generator=generator)
...
def parse_args():
    ...
    parser.add_argument(
        "--unet_condition_model_dir",
        type=str,
        default=None,
        help=(
            "unet pretrain model dir for condition feature extract"),
    )
...
def main():
    ...
    unet_cond = UNet2DConditionModel.from_pretrained(args.unet_condition_model_dir, subfolder="unet", revision=args.non_ema_revision)
    ...
    unet = RefOnlyNoisedUNet(unet=_unet_, unet_cond=unet_cond, train_sched=noise_scheduler, val_sched=val_noise_scheduler)

    # Freeze vae and text_encoder and set unet to trainable
    vae.requires_grad_(False)
    vision_encoder.requires_grad_(False)
    text_encoder.requires_grad_(False)
    unet_cond.requires_grad_(False)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                cond_image_out = batch["cond_image_out"]
                cond_image_vae = scale_image(cond_image_vae)
                images_out = scale_image(images_out)
                cond_image_out = scale_image(cond_image_out)
                b, v, c, h, w = images_out.shape

                train_cond = False
                prob_train_cond = 0.3
                if random.random() < prob_train_cond:
                    train_cond = True
                    b, c, h, w = cond_image_out.shape
                    latents_out_cond = vae.encode(cond_image_out.to(weight_dtype)).latent_dist.sample()
                    latents_out_grid = latents_out_cond
                ...
                if args.do_classifier_free_guidance:
                    ...
                    drop_idx = np.where(np.random.rand(cond_image_vae.shape[0])<0.2)
                ...
                latents_cond_vae = latents_cond_vae * vae.config.scaling_factor
                latents_cond_vae = scale_latents(latents_cond_vae)
