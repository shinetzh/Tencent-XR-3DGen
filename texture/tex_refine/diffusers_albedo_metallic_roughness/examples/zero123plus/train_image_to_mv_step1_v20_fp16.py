...
from src.diffusers.pipelines.zero123_diffusion.pipeline_zero123plus_v1 import Zero123PlusPipeline, RefOnlyNoisedUNet, RefOnlyNoisedUNetCond, scale_image, scale_latents, unscale_image, unscale_latents
...
def main():
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        ...
        unet_cond_UNet = UNet2DConditionModel.from_pretrained(
            args.pretrained_model_name_or_path, subfolder="unet_cond", revision=args.revision
        )
        unet_cond_UNet.set_attention_slice(slice_size="auto")
        unet_cond = RefOnlyNoisedUNetCond(unet=unet_cond_UNet, train_sched=noise_scheduler, val_sched=val_noise_scheduler)
    ...
    unet_cond.to(accelerator.device, dtype=weight_dtype)
    ...
    ramping_coefficients_weight = torch.tensor(ramping_coefficients, dtype=torch.float32).to(accelerator.device)
    ramping_coefficients_weight.requires_grad = True

    optimizer = optimizer_cls([
        {'params': filter(lambda p: p.requires_grad, unet.parameters())},
        {'params': [ramping_coefficients_weight]}],
    ...
    unet, optimizer, train_dataloader, lr_scheduler, ramping_coefficients_weight = accelerator.prepare(
        unet, optimizer, train_dataloader, lr_scheduler, ramping_coefficients_weight
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                encoder_hidden_states = encoder_hidden_states_prompt + cond_encoded_clip * ramping_coefficients_weight
