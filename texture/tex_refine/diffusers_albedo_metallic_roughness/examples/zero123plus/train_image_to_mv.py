...
from src.diffusers.pipelines.zero123_diffusion.pipeline_zero123plus import Zero123PlusPipeline, RefOnlyNoisedUNet, scale_image, scale_latents, unscale_image, unscale_latents
...
def log_validation(vae, text_encoder, vision_encoder, tokenizer, unet, args, accelerator, weight_dtype, epoch, max_num=8):
    ...
    for i in range(len(validation_image_path_list)):
        ...
        with torch.autocast("cuda"):
            ...
            result = pipeline(cond, num_inference_steps=75, generator=generator, output_type="pt")
        if result is None:
            continue
    ...
    nrow = 4
    images_cond_grid = make_grid(images_pred_all, nrow=nrow, padding=1, value_range=(0, 1))
...
def main():
    ...
    with open(model_index_path, "r") as fr:
        model_index_dict = json.load(fr)
        ramping_coefficients = np.array(model_index_dict["ramping_coefficients"])
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                nrow = v // 2
                ncol = v // nrow
                latents_out_grid = torch.zeros((b, cl, hl*nrow, wl*ncol)).to(latents_out.device, dtype=weight_dtype)
                for i in range(b):
                ...
                ##### TODO: can change to be learn
                ramp = cond_encoded_clip.new_tensor(ramping_coefficients).unsqueeze(-1)
                encoder_hidden_states = encoder_hidden_states_prompt + cond_encoded_clip * ramp

                if args.do_classifier_free_guidance:
                    negative_cond_image_vae = torch.zeros_like(cond_image_vae, dtype=weight_dtype)
                    drop_idx = np.where(np.random.rand(cond_image_vae.shape[0])<0.1)
                ...
                # for name, param in unet.named_parameters():
                #     if name == params_require_grad[10]:
