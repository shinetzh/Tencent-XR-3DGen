...
def log_validation(vae, text_encoder, vision_encoder, tokenizer, unet_cond, unet, args, accelerator, weight_dtype, epoch, max_num=8):
    ...
    pipeline = Zero123PlusPipeline.from_pretrained(
        ...
        unet=accelerator.unwrap_model(unet).unet.clone().half(),
...
def main():
    ...
    vision_encoder.to(accelerator.device, dtype=torch.float16)
    text_encoder.to(accelerator.device, dtype=torch.float16)
    vae.to(accelerator.device, dtype=torch.float16)
    unet_cond.to(accelerator.device, dtype=torch.float16)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                latents_out_grid = vae.encode(images_out_grid.to(torch.float16)).latent_dist.sample()
                ...
                cond_encoded_clip = vision_encoder(cond_image_clip.to(torch.float16), output_hidden_states=False) ### clip with projection
                ...
                latents_cond_vae = vae.encode(cond_image_vae.to(torch.float16)).latent_dist.sample()
                cross_attention_kwargs = dict(cond_lat=latents_cond_vae.to(weight_dtype))
                cross_attention_kwargs_cond = dict(cond_lat=latents_cond_vae)
                ...
                for key, _ in ref_dict.items():
                    ref_dict[key] = ref_dict[key].to(weight_dtype)

                model_pred = unet(
                    noisy_latents.to(weight_dtype),
                    timesteps,
                    encoder_hidden_states_prompt=encoder_hidden_states_prompt.to(weight_dtype),
                    cond_encoded_clip=cond_encoded_clip.to(weight_dtype),
                    drop_idx=drop_idx,
