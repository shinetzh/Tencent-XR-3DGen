...
def main():
    ...
    # print(unet)
    # print(unet.conv_out.in_channels)
    # print(unet.conv_out.out_channels)
    # print(unet.conv_out.kernel_size)
    # print(unet.conv_out.stride)
    # print(unet.conv_out.padding)
    # breakpoint()
    ...
    conv_out_out_channels = 8
    unet.register_to_config(out_channels=conv_out_out_channels)
    ...
    with torch.no_grad():
        ...
        # new_conv_in.weight[:, :16, :, :].copy_(unet.conv_in.weight)
        unet.conv_in = new_conv_in

        new_conv_out = nn.Conv2d(
            unet.conv_out.in_channels, conv_out_out_channels, unet.conv_out.kernel_size, unet.conv_out.stride, unet.conv_out.padding
            )
        
        # print(new_conv_out.in_channels)
        # print(unet.conv_out.in_channels)

        # print(new_conv_out.out_channels)
        # print(unet.conv_out.out_channels)

        # print(new_conv_out.weight.shape)
        # print(unet.conv_out.weight.shape)

        new_conv_out.weight.zero_()
        # new_conv_out.weight[:unet.conv_out.out_channels, :, :, :].copy_(unet.conv_out.weight)
    ...
    print(configs["data_config"]["dataset_name"])
    # breakpoint()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                albedos = batch["albedos"]
                metallic_roughness = batch["metallics_roughness"]
                ...
                albedos_grid = torch.zeros((b, c, h*ncol, w*nrow)).to(images.device, dtype=weight_dtype)
                metallic_roughness_grid = torch.zeros((b, c, h*ncol, w*nrow)).to(images.device, dtype=weight_dtype)

                for i in range(b):
                    images_grid[i] = make_grid(images[i], nrow=nrow, padding=0)
                    # roughness_grid[i] = make_grid(roughnesses[i], nrow=nrow, padding=0)
                    # metallic_grid[i] = make_grid(metallics[i], nrow=nrow, padding=0)
                    albedos_grid[i] = make_grid(albedos[i], nrow=nrow, padding=0)
                    metallic_roughness_grid[i] = make_grid(metallic_roughness[i], nrow=nrow, padding=0)
                ...
                latents_input = torch.cat((latents_images_grid, latents_images_grid), dim=1)
                ...
                # latents_metallic_grid = vae.encode(metallic_grid.to(weight_dtype)).latent_dist.sample()
                # latents_metallic_grid = latents_metallic_grid * vae.config.scaling_factor
                # latents = latents_metallic_grid

                latents_albedos_grid = vae.encode(albedos_grid.to(weight_dtype)).latent_dist.sample()
                latents_albedos_grid = latents_albedos_grid * vae.config.scaling_factor

                latents_metallic_roughness_grid = vae.encode(metallic_roughness_grid.to(weight_dtype)).latent_dist.sample()
                latents_metallic_roughness_grid = latents_metallic_roughness_grid * vae.config.scaling_factor

                # latents_albedo = latents_albedos_grid

                latents_albedo = torch.cat((latents_albedos_grid, latents_metallic_roughness_grid), dim=1)
                # print(latents_albedo.shape)
                # breakpoint()

                # latents_roughness_grid = vae.encode(roughness_grid.to(weight_dtype)).latent_dist.sample()
                # latents_roughness_grid = latents_roughness_grid * vae.config.scaling_factor
                # latents = latents_roughness_grid
                ...
                noise = torch.randn_like(latents_albedo)
                # breakpoint()
                if args.noise_offset:
                    # https://www.crosslabs.org//blog/diffusion-with-offset-noise
                    noise += args.noise_offset * torch.randn(
                        (latents_albedo.shape[0], latents_albedo.shape[1], 1, 1), device=latents_albedo.device
                    )
                if args.input_perturbation:
                    new_noise = noise + args.input_perturbation * torch.randn_like(noise)
                bsz = latents_albedo.shape[0]
                # Sample a random timestep for each image
                timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps, (bsz,), device=latents_albedo.device)
                timesteps = timesteps.long()

                # Add noise to the latents according to the noise magnitude at each timestep
                # (this is the forward diffusion process)
                if args.input_perturbation:
                    noisy_latents = noise_scheduler.add_noise(latents_albedo, new_noise, timesteps)
                else:
                    noisy_latents = noise_scheduler.add_noise(latents_albedo, noise, timesteps)
                ...
                if noise_scheduler.config.prediction_type == "epsilon":
                    ...
                    target = noise_scheduler.get_velocity(latents_albedo, noise, timesteps)
                elif noise_scheduler.config.prediction_type == "sample":
                    target = latents_albedo
                else:
                    raise ValueError(f"Unknown prediction type {noise_scheduler.config.prediction_type}")

                #### add condition concat
                noisy_latents = torch.cat([noisy_latents, latents_input], dim=1)
                ...
                # res_pred_albedo = vae.decode((model_pred[:,:4, :, :] / vae.config.scaling_factor).to(weight_dtype), return_dict=False)[0]
                # # res_pred_metallic_roughness = vae.decode((model_pred[:,:4, :, :] / vae.config.scaling_factor).to(weight_dtype), return_dict=False)[0]
                # # res_pred_bumpmap = vae.decode((model_pred[:,:4, :, :] / vae.config.scaling_factor).to(weight_dtype), return_dict=False)[0]
                # print(res_pred_albedo.shape)
                # # print(res_pred_metallic_roughness.shape)
                # # print(res_pred_bumpmap.shape)
                # breakpoint()
