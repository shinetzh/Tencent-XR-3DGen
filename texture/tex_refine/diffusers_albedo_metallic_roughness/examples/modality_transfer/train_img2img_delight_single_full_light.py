    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                latents_images = vae.encode(images.to(weight_dtype)).latent_dist.sample()
                latents_images = latents_images * vae.config.scaling_factor

                latents_gt = vae.encode(gt.to(weight_dtype)).latent_dist.sample()
                ...
                noisy_latents = torch.cat([noisy_latents, latents_images], dim=1)
