...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, time_embedding, condition_embedding):
        ...
        ## add by neoshang
        x = self.attn_cross(x, condition_embedding)
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.condition_embedder = nn.Sequential(
            nn.Linear(condiiton_dim, hidden_size, bias=True),
            nn.SiLU(),
            nn.Linear(hidden_size, hidden_size, bias=True),
        )
    ...
    def save_pretrained(self, save_dir, weights_name="dit.pt"):
        # Save the model
        state_dict = self.state_dict()
        torch.save(state_dict, os.path.join(save_dir, weights_name))

    def load_pretrained(self, save_dir, weights_name="dit.pt"):
        # Load the model
        state_dict = torch.load(os.path.join(save_dir, weights_name), map_location="cpu")
...
if __name__ == "__main__":
    ...
    test dit
    """
    dit = DiT_S_1(input_size=16, in_channels=4, condiiton_dim=768, learn_sigma=False).cuda()
    x = torch.randn([5, 4, 16, 48]).cuda()
    y = torch.randn([5, 197, 768]).cuda()
