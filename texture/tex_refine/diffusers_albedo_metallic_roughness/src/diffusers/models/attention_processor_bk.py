...
class Attention(nn.Module):
    ...
    def set_attention_slice(self, slice_size):
        ...
        if slice_size is not None and self.added_kv_proj_dim is not None:
            # print("SlicedAttnAddedKVProcessor")
            processor = SlicedAttnAddedKVProcessor(slice_size)
        elif slice_size is not None:
            # print("SlicedAttnProcessor")
...
class SlicedAttnProcessor:
    ...
    def __call__(self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        for i in range((batch_size_attention - 1) // self.slice_size + 1):
            ...
            # ##### use checkpoint
            # attn_slice = torch.utils.checkpoint.checkpoint(attn.get_attention_scores, *(query_slice, key_slice, attn_mask_slice), use_reentrant=False)
            # attn_slice = torch.utils.checkpoint.checkpoint(torch.bmm, *(attn_slice, value[start_idx:end_idx]), use_reentrant=False)
            hidden_states_list.append(attn_slice)
