...
from utils_pix import auto_grad_checkpoint, to_2tuple
from dit_pix_blocks import t2i_modulate, CaptionEmbedder, WindowAttention, MultiHeadCrossAttention, T2IFinalLayer, TimestepEmbedder, LabelEmbedder, FinalLayer
...
class PixArt(nn.Module):
    ...
    def __init__(self, input_length=512, input_dim=8, cond_length=197, cond_dim=768, 
                 hidden_size=1152, depth=24, num_heads=16, mlp_ratio=4.0, 
                 class_dropout_prob=0.1, pred_sigma=True, drop_path: float = 0., 
                  use_rel_pos=False, lewei_scale=1.0, config=None, **kwargs):
        ...
        self.out_channels = input_dim * 2 if pred_sigma else input_dim
        self.num_heads = num_heads
        self.lewei_scale = lewei_scale,

        self.x_embedder = nn.Linear(input_dim, hidden_size, bias=True)
        self.t_embedder = TimestepEmbedder(hidden_size)

        # Will use fixed sin-cos embedding:
        self.register_buffer("pos_embed", torch.zeros(1, input_length, hidden_size))
        ...
        self.y_embedder = CaptionEmbedder(in_channels=cond_dim, hidden_size=hidden_size, 
                                          uncond_prob=class_dropout_prob, act_layer=approx_gelu, 
                                          token_num=cond_length)
