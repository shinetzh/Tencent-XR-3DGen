...
class Zero123PlusImg2ImgPipeline(diffusers.StableDiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs
        ...
        latents_in_grid = latents_in_grid.to(self.vae.device)
        class_embeddings = class_embeddings.to(self.vae.device)
        global_embeds = cond_encoded_clip.to(self.vae.device)
        encoder_hidden_states_prompt = encoder_hidden_states_prompt.to(self.vae.device)

        if guidance_scale > 1:
            latents_in_grid = torch.cat([torch.zeros_like(latents_in_grid), latents_in_grid])

        if "ramping_coefficients" in self.unet.config:
            encoder_hidden_states = self.unet.ramping_add(encoder_hidden_states_prompt, global_embeds)
        else:
