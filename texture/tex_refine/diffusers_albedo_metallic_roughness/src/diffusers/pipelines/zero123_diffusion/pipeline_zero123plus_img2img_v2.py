...
class ImageConditionUNet(torch.nn.Module):
...
class DepthControlUNet(torch.nn.Module):
    def __init__(self, unet: ImageConditionUNet, controlnet: Optional[diffusers.ControlNetModel] = None, conditioning_scale=1.0) -> None:
        super().__init__()
        self.unet = unet
...
class Zero123PlusImg2ImgPipeline(diffusers.StableDiffusionPipeline):
    ...
    def prepare(self):
        ...
        if not isinstance(self.unet, ImageConditionUNet):
            print("prepare unet of ImageConditionUNet")
            self.unet = ImageConditionUNet(self.unet, train_sched, self.scheduler).eval()
    ...
    def __call__(
        ...
        **kwargs
        ...
        images_in_process_list = []
        for image_in in images_in:
            image_in = to_rgb_image(image_in, 255)
            image_in = self.train_transforms(image_in)
            images_in_process_list.append(image_in.squeeze())
        images_in = torch.stack(images_in_process_list, dim=0)


        images_in = scale_image(images_in)
        v, c, h, w = images_in.shape
        b = 1
        latents_in = self.vae.encode(images_in.view(b*v, c, h, w).to(self.vae.device)).latent_dist.sample()
        _, cl, hl, wl = latents_in.shape
        latents_in = latents_in.view(b, v, cl, hl, wl)
        ncol = v // 2
        nrow = v // ncol
        latents_in_grid = torch.zeros((b, cl, hl*ncol, wl*nrow)).to(self.vae.device)
        for i in range(b):
            latents_in_grid[i] = make_grid(latents_in[i], nrow=nrow, padding=0)
        # assert torch.sum(latents_in_grid[2, :, 40:, :40] - latents_in[2, 2, ...]) == 0
        latents_in_grid = latents_in_grid * self.vae.config.scaling_factor
        ...
        if guidance_scale > 1:
            latents_in_grid = torch.cat([torch.zeros_like(latents_in_grid), latents_in_grid])
        ...
        # encoder_hidden_states = torch.load("/aigc_cfs_2/neoshang/code/diffusers_triplane/encoder_hidden_states_prompt.pt").to(self.vae.device)
        ...
        # class_embeddings = torch.load("/aigc_cfs_2/neoshang/code/diffusers_triplane/class_embeddings.pt").to(self.vae.device)
        ...
        result_image = self.image_processor.postprocess(image, output_type="pt")
    ...
    def stable_diffusion_call(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                class_embeddings_input = torch.cat([class_embeddings]*2) if do_classifier_free_guidance else class_embeddings
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                latent_model_input = torch.cat([latent_model_input, latents_in], dim=1)
                
                # predict the noise residual
                noise_pred = self.unet(
                    latent_model_input,
                    t,
                    class_labels=class_embeddings_input,
