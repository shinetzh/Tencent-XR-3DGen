...
def init_models_texture(
    ...
    # pretrained_weight_path = "../../../../Tencent-XR-3DGen/weights/"
    # pretrained_weight_path = "../Tencent-XR-3DGen/weights/"
    pretrained_weight_path = f"{weights_path}/weights/"
    ...
    args.pretrained_model_name_or_path = f"{pretrained_weight_path}/helloworldXL70"
    ...
    if three_views_input:
        ...
        args.xyz_controlnet_path = f"{pretrained_weight_path}/pretrained_sdxl_xyz_controlnet"
    ...
    model_path=f"{pretrained_weight_path}/realesrgan/realesr-general-x4v3.pth"
    ...
    model_path = f"{pretrained_weight_path}/realesrgan/RealESRGAN_x2plus.pth"
    ...
    if args.three_views_input:
        ...
        args.z123_model_path = os.path.join(pretrained_weight_path, "zero23plus_v25_4vews_abs_39000") # 3views model
        args.controlnet_model_path = os.path.join(pretrained_weight_path, "zero23_controlnet_8000")
        ...
        args.z123_model_path = f"{pretrained_weight_path}/zero123plus_v29_1cond_6views_090180270"
        ...
        args.controlnet_model_path = f"{pretrained_weight_path}/zero123plus_v29_1cond_6views_090180270_controlnet_15000"
    ...
    albedo_pipeline = PBRPipeline.from_pretrained(
                    ...
                    f"{pretrained_weight_path}/img2img_albedo_6views_512_250k",
    ...
    metallic_roughness_pipeline = PBRPipeline.from_pretrained(
                    ...
                    f"{pretrained_weight_path}/img2img_material_two_outputs",
                    torch_dtype=torch.float16,
                    local_files_only=True
                )
    ...
    ref_albedo_pipeline = SingleAlbedoPipeline.from_pretrained(
                        ...
                        f"{pretrained_weight_path}/delight_models_full_light",
                        torch_dtype=torch.float16,
                        local_files_only=True
                    )
    ...
    vae = AutoencoderKL.from_pretrained(f"{pretrained_weight_path}/sdxl-vae-fp16-fix", torch_dtype=torch.float16)
    ...
    # tile controlnet
    controlnet_tile = ControlNetModel.from_pretrained(f"{pretrained_weight_path}/TTPLANET_Controlnet_Tile/", torch_dtype=weight_dtype)
    # depth controlnet
    controlnet_depth = ControlNetModel.from_pretrained(f"{pretrained_weight_path}/sdxl_depth_controlnet_fp16/", torch_dtype=weight_dtype)
    ...
    print("lora...")
    if "XDetail_light" in args.sdxl_lora_stack:
        pipeline.load_lora_weights(pretrained_weight_path, weight_name='XDetail_light.safetensors', adapter_name="XDetail_light")
    if "contrast_tool" in args.sdxl_lora_stack:
        pipeline.load_lora_weights(pretrained_weight_path, weight_name='SDS_Contrast tool_XL.safetensors', adapter_name="contrast_tool")
    if "sdxl_lightning_8step_lora" in args.sdxl_lora_stack:
        pipeline.load_lora_weights(pretrained_weight_path, weight_name='sdxl_lightning_8step_lora.safetensors', adapter_name="sdxl_lightning_8step_lora")
    if "xl_color_temp" in args.sdxl_lora_stack:
        pipeline.load_lora_weights(pretrained_weight_path, weight_name='xl_color_temp.safetensors', adapter_name="xl_color_temp")
    # Activate the LoRA
    # # Activate the LoRA
    ...
    image_encoder_path = os.path.join(pretrained_weight_path, "IP-Adapter_image_encoder")
    ip_ckpt = os.path.join(pretrained_weight_path, "ip-adapter-plus_sdxl_vit-h.bin")
...
def inference_texture(
    ...
    output_path = os.path.join(out_dir, "outputs_service_d2rgb_6views_sdxl_sr_local")
    os.makedirs(output_path, exist_ok=True)
    output_path = os.path.join(output_path, current_time[:10],current_time+"_"+str(job_id))
    ...
    if input_has_uv:
        ...
        print(f"{out_dir}/{basename}")
        os.system(f"cp {mesh[:-3]+'obj'} {out_dir}/{basename}")
        ...
        mesh_export_path = f"{out_dir}/{basename}"
        rgb_texture = sdxl_pipeline.scheduler.last_texture_map
        mrz_texture = F.interpolate(metallic_roughness_z_texture.permute(0,3,1,2), size=(rgb_texture.shape[0], rgb_texture.shape[1]), mode='bicubic')[0].permute(1,2,0)
        pbr_texture = torch.cat([rgb_texture, mrz_texture[:,:,1:2],mrz_texture[:,:,0:1]], dim=-1)
        sdxl_pipeline.scheduler.renderer.export_mesh(mesh_export_path, pbr_texture)
...
if __name__ == '__main__':
    ...
    models = init_models_texture(three_views_input=False, debug_outputs=True)
    ...
    for repeat_time in range(args.repeat):
        ...
        output_path = os.path.join(args.output_path, "texture_dir")
        os.makedirs(output_path, exist_ok=True)
