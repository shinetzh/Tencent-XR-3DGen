...
from utils.utils_obj import export_obj,export_obj_gradio
...
def init_models_shape(weights_path):
    # pretrained_weights_path = "/root/autodl-tmp/xibin/code/Tencent-XR-3DGen/"
    pretrained_weights_path = weights_path

    configs_path = os.path.join("./geometry/main_pipeline/diffusion/configs/1view_gray_2048_flow", "train_configs.json")
    ...
    configs["vae_config"]["pretrain_path"] = f"{pretrained_weights_path}/geometry_vae/model.ckpt"
    ...
    configs["dino_config"]["pretrain_dir"] = f"{pretrained_weights_path}/dinov2-large"
    ...
    configs["clip_config"]["pretrain_dir"] = f"{pretrained_weights_path}/CLIP-ViT-bigG-14-laion2B-39B-b160k"
    ...
    ckpt_dir = "/root/autodl-tmp/xibin/code/Tencent-XR-3DGen/geometry_dit/checkpoint/unet_ema"
    ...
    models = {
        'latent_sequence_num':latent_sequence_num,
        'vae':vae,
...
@torch.no_grad()
def inference_shape(models, image_path, save_test_dir):
    num = 0
    weight_dtype = models["weight_dtype"]
    latent_sequence_num = models["latent_sequence_num"]
    vae = models["vae"]

    def get_dino_feature(images_list):
        dino_image_processor = models["dino_image_processor"]
    ...
    dino_empty_local_path = f"{pretrained_weights_path}/zero_image_features/gray_image_local_dino_embedding.pt"
    clip_empty_global_path = f"{pretrained_weights_path}/zero_image_features/gray_image_global_clip_embedding.pt"
    ...
    if True:
        ...
        mesh_name = os.path.join(save_test_dir, objname + ".obj")
        ...
        try:
            ...
            # continue
            print("error occur with image segmentation !")

        image_save_path = os.path.join(save_test_dir, os.path.basename(image_path))
        image.save(image_save_path)

        image_latents_local_dino = get_dino_feature([image])

        # print(models["dino_image_processor"])
        ...
        # dino_model.to(torch.device("cpu"))
        # # torch.cuda.empty_cache()
        # image_latents_local_dino = last_hidden_states
        ...
        export_obj_gradio(vertices_sparse,
            faces_sparse[:, ::-1], mesh_name)
...
def test(args):
    ...
    save_test_dir = args.save_dir

    # pretrained_weights_path = "../../../../Tencent-XR-3DGen/"
    # pretrained_weights_path = "../Tencent-XR-3DGen/"
    pretrained_weights_path = "./peterji/Tencent_XR_3DGen/"
    ...
    # dino_empty_local_path = "geometry/main_pipeline/diffusion/configs/1view_gray_2048_flow/gray_image_local_dino_embedding.pt"
    # clip_empty_global_path = "geometry/main_pipeline/diffusion/configs/1view_gray_2048_flow/gray_image_global_clip_embedding.pt"
