...
class DiffusionModelOneStageUncond(nn.Module):
    def __init__(
        self,
        model,
    ...
    def sample(self, dim, batch_size, noise=None, clip_denoised = "static", traj=False, cond=None, class_free_weight=0):
        ...
        # x_T = default(noise, torch.randn(batch, (3, 32, 128, 128), device = device))
        x_T = torch.randn((batch, 3, 32, 128, 128), device = device)
    ...
    def diffusion_model_from_latent(self, x_start, cond=None):
        ...
        return loss, loss_100, loss_1000, model_out, t
    ...
    def generate_unconditional(self, num_samples):
        ...
        with torch.no_grad():
            ...
            samp,_ = self.sample(dim=(3, 32, 128, 128), batch_size=num_samples, traj=False, cond=None)
