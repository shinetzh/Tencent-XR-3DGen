...
class FPN_down(nn.Module):
    def __init__(self,in_channel_list, out_channel_list, conv_group=1):
        super(FPN_down, self).__init__()
        self.inner_layer=nn.ModuleList()
        self.out_layer=nn.ModuleList()

        for i, in_channel in enumerate(in_channel_list[:-1]):
            self.inner_layer.append(nn.Conv2d(in_channel, out_channel_list[i],1, groups=conv_group).cuda())
            self.out_layer.append(nn.Conv2d(out_channel_list[i],out_channel_list[i],kernel_size=3,padding=1, groups=conv_group).cuda())
        # self.upsample=nn.Upsample(size=, mode='nearest')
...
if __name__ == '__main__':
    ...
    # fpn=FPN([10,20,30],5).cuda()
    # x=[]
    # x.append(torch.rand(1, 10, 64, 64).cuda())
    # x.append(torch.rand(1, 20, 16, 16).cuda())
    # x.append(torch.rand(1, 30, 8, 8).cuda())
    # c=fpn(x)
    # print(c)

    # features_down_dim = [512, 512, 1024, 1024]
    # features_out_dim = [512, 1024, 1024]

    # features_size = [64, 32, 16, 8]
    # features_down = []
    ...
    # fpn_down = FPN_down(features_down_dim, features_out_dim)

    # features_output = fpn_down(features_down)
