...
class Embedder:
    ...
    def embed(self, inputs, alpha_ratio=0.):
        ...
        if self.kwargs['include_input']:
            start = 1
        for i in range(self.kwargs['num_freqs']):
            output[:, (self.num_fns*i+start)*self.kwargs['input_dims']:(self.num_fns*(i+1)+start)*self.kwargs['input_dims']] *= (1.-math.cos(
                math.pi*(max(min(alpha_ratio*self.kwargs['num_freqs']-i, 1.), 0.))
            )) * .5
        return output
