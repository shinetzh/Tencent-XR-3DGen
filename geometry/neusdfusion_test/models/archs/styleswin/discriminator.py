...
from models.archs.styleswin.op import FusedLeakyReLU, upfirdn2d
from models.archs.styleswin.basic_layers import (Blur, Downsample, EqualConv2d, EqualLinear,
                                 ScaledLeakyReLU)
...
class FromRGB(nn.Module):
    def __init__(self, out_channel, downsample=True, blur_kernel=[1, 3, 3, 1], sn=False):
        ...
        self.conv = ConvLayer(96 * 4, out_channel, 1, sn=sn)
...
class Discriminator(nn.Module):
    def __init__(self, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], sn=False, ssd=False):
        ...
        if sn:
            ...
            self.final_linear = nn.Sequential(
                EqualLinear(channels[4] * 4 * 4, channels[4] * 4, activation='fused_lrelu'),
                EqualLinear(channels[4] * 4, 2048),
            )
