...
class ConvPointnet(nn.Module):
    ...
    def generate_plane_features(self, p, c, plane='xz'):
        ...
        xy = self.normalize_coordinate2(p.clone(), plane=plane, padding=self.padding) # normalize to the range of (0, 1)
    ...
    def forward(self, p, query):
        ...
        if 'xz' in self.plane_type:
            coord['xz'] = self.normalize_coordinate2(p.clone(), plane='xz', padding=self.padding)
            index['xz'] = self.coordinate2index(coord['xz'], self.reso_plane)
        if 'xy' in self.plane_type:
            coord['xy'] = self.normalize_coordinate2(p.clone(), plane='xy', padding=self.padding)
            index['xy'] = self.coordinate2index(coord['xy'], self.reso_plane)
        if 'yz' in self.plane_type:
            coord['yz'] = self.normalize_coordinate2(p.clone(), plane='yz', padding=self.padding)
    ...
    def sample_plane_feature(self, query, plane_feature, plane):
        xy = self.normalize_coordinate2(query.clone(), plane=plane, padding=self.padding)
        xy = xy[:, :, None].float()
