import sys
sys.path.append("/apdcephfs_cq8/share_2909871/trevorrkcui/projects/DiffusionSDF")
import math
import torch
...
class DiffusionNetLatentUncond(nn.Module):
    ...
    def forward(
        ...
        if self.cross_attn:
            ...
            tokens = self.causal_transformer(tokens, context=None)
        else:
...
if __name__ == '__main__':
   net = CausalTransformer(dim = 4096, dim_in_out=4096, depth=4).cuda()
   z = torch.rand(2, 4096).cuda()
   out = net(z)
   print(out)
