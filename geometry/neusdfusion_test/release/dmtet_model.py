...
def tar_process(save_path, target_dir):
  target_dir_parent = os.path.dirname(target_dir)
  target_name = os.path.basename(target_dir)
  os.system("tar -cvf {} -C {} {}".format(save_path, target_dir_parent, target_name))
...
class DmtetModel:
  ...
  def render_image(self, plane_features, mesh_list, num=30, view_max=61):
    print("render_image...")
    ...
    while start < objs_num_all:
      ...
      np_imgs = self.model.decoder_model.render_image(self.glctx, plane_features[start:end], mesh_list[start:end], target_cam_positions)
      start = end
  ...
  def get_front_images(self, plane_features, mesh_list):
    print("get_front_image...")
    imgs_cat = self.render_image(plane_features, mesh_list, num=1)
    return imgs_cat
  ...
  def save_mesh_gif(self, plane_features, mesh_list):
    ...
    imgs_cat = self.render_image(plane_features, mesh_list, num = 30)
    ...
    for idx, (image_list, mesh) in enumerate(zip(images_list, mesh_list)):
        ...
        tar_process(save_tar_path, save_dir)
  ...
  @torch.no_grad()
  def generate_triplane_mesh(self, latent, max_face_num=100000, batch_size=8, progress=None):
      ...
      mesh_list = self.model.decoder_model.xatlas_uvmap_batch(self.glctx, plane_features, max_face_num, progress)
